...
from .http_client_async import AioHttpClient
...
class LineBotApiAsync(object):
    ...
    def __init__(self, channel_access_token, endpoint=DEFAULT_API_ENDPOINT,
                 timeout=HttpClient.DEFAULT_TIMEOUT, http_client=None):
        """__init__ method.
        ...
        :param http_client: (optional) Default is
            :py:class:`linebot.http_client.AioHttpClient`
    ...
    def get_rich_menu(self, rich_menu_id, timeout=None):
        ...
        json_resp = yield from response.json
    ...
    def create_rich_menu(self, rich_menu, timeout=None):
        ...
        :return: rich menu id
        """
        response = yield from self._post(
    ...
    def _get(self, path, params=None, stream=False, timeout=None):
        ...
        response = yield from self.http_client.get(
            url, headers=self.headers, params=params, stream=stream, timeout=timeout
        )

        yield from self.__check_error(response)
        return response
    ...
    def _post(self, path, data=None, timeout=None):
        ...
        response = yield from self.http_client.post(
    ...
    def _delete(self, path, data=None, timeout=None):
        ...
        response = yield from self.http_client.delete(
            url, headers=headers, data=data, timeout=timeout
        )
    ...
    def __check_error(response):
        if 200 <= response.status_code < 300:
            ...
            error = Error.new_from_json_dict((yield from response.json))
